...
async def _async_test_concurrency():
    ...
    try:
        ...
        async with httpx.AsyncClient(transport=transport, base_url="http://testServer") as client:
            ...
            try:
                ...
                from models import Booking, Procurement, Payment, QualityCheck, Weighment, AuditLogEntry
                
                # Cascading deletes
                for b in db.query(Booking).filter(Booking.farmer_id == farmer.id).all():
                    db.query(AuditLogEntry).filter(AuditLogEntry.booking_id == b.id).delete()
...
def test_concurrency():
    asyncio.run(_async_test_concurrency())
